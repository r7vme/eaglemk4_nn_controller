...
class ConvVAE(object):
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            # Encoder
            h = tf.layers.conv2d(self.x, 32, 4, strides=2, padding="same",
                                 activation=tf.nn.relu, name="enc_conv1")
            h = tf.layers.conv2d(h, 64, 4, strides=2, padding="same",
                                 activation=tf.nn.relu, name="enc_conv2")
            h = tf.layers.conv2d(h, 128, 4, strides=2, padding="same",
                                 activation=tf.nn.relu, name="enc_conv3")
            h = tf.layers.conv2d(h, 256, 4, strides=2, padding="same",
            ...
            h = tf.reshape(h, [-1, 5, 10, 256])
            h = tf.layers.conv2d_transpose(h, 128, 4, strides=2,
                                           padding="same",
                                           activation=tf.nn.relu,
                                           name="dec_deconv1")
            h = tf.layers.conv2d_transpose(h, 64, 4, strides=2,
                                           padding="same",
                                           activation=tf.nn.relu,
                                           name="dec_deconv2")
            h = tf.layers.conv2d_transpose(h, 32, 5, strides=2,
                                           padding="same",
                                           activation=tf.nn.relu,
                                           name="dec_deconv3")
            self.y = tf.layers.conv2d_transpose(h, 3, 4, strides=2,
                                                padding="same",
